---
title: リソースリスト
date: 2025-07-17
lastmod: 2025-07-17
description: リソースリストは、共通した複数の任意情報で構成されたオブジェクトのリストを表示します。
storybook: https://cieloazul310.github.io/digital-go-design-system-with-panda/?path=/docs/components-%E3%83%AA%E3%82%BD%E3%83%BC%E3%82%B9%E3%83%AA%E3%82%B9%E3%83%88--docs
digitalgo: https://design.digital.go.jp/dads/components/resource-list/
---

```tsx
import { ResourceList } from "@/components/ui/resource-list";
```

## 使用例

<DemoViewer name="resource-list" />

### リンク

`<ResourceList.Root>`に`asLink`属性を付与すると、ホバー時にインタラクションが加わる。<del>このとき、通常`<ResourceList.Title>`に`asChild`属性を付与し、子要素のリンクと合成して使用する。</del>

<ins>v0.3以降を使用する場合は、`<ResourceList.Title>`に`asChild`属性を付与しないでください</ins>

<DemoViewer name="resource-list-aslink" />
